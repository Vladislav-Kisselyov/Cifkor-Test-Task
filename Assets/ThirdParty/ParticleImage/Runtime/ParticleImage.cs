        ...
        private void Simulate(float deltaTime, bool prewarming = false)
        {
            ...
            if (isPlaying)
            {
                ...
                if (multithreadEnabled)
                {
                    ...
                    var totalIndices = 0;
                    const int MAX_TOTAL_TRAIL_VERTICES = 65000;
                    ...
                    if (trailsEnabled && _particleTrailRenderer)
                    {
                        for (int i = 0; i < JobParticles.Length; i++)
                        {
                            ...
                            {
                                int particleVertices = TrailData[i].Length * 2;
                                int particleIndices = (TrailData[i].Length - 1) * 6;

                                if (totalTrailPoints + particleVertices > MAX_TOTAL_TRAIL_VERTICES)
                                {
                                    p.trailVertexOffset = -1;
                                    JobParticles[i] = p;
                                    continue;
                                }

                                totalTrailPoints += particleVertices;
                                totalIndices += particleIndices;
                            }
                        }
